...
func (this *ReaderHandlerFixture) Setup() {
	...
	this.writeLine(header) // write header
}

// instead of reading from the physical file they are reading from the buffer
// and sending to to the output channel [logic is done in the Handle function]
...
func (this *ReaderHandlerFixture) assertCleanUp() {
	this.So(<-this.output, should.Equal, endOfFile)
